...
namespace SenoraRP_Chatlog_Assistant.Utilities
{
        ...
        public static void SaveParsedHash(string log)
        {
            using (MD5 md5Hash = MD5.Create())
            {
                ...
                if (Properties.Settings.Default.SameHashAutoCount >= Properties.Settings.Default.SameHashWarnAmount)
                    MessageBox.Show(string.Format("The same chat log has been backed up {0} or more times, which means that the RAGEMP client script responsible for logging your chat is broken.\r\n\r\nThis happens when you lose internet connection or the server crashes. The only known way of fixing the issue is deleting the resource directories for the server you are playing on.\r\n\r\nLost chat logs cannot be recovered.", Properties.Settings.Default.SameHashWarnAmount), "Warning", MessageBoxButton.OK, MessageBoxImage.Warning);
            }
        }
