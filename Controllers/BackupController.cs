...
using SenoraRP_Chatlog_Assistant.Utilities;
        ...
        private static void ParseThenSaveToFile(bool gameClosed = false)
        {
            try
            {
                ...
                if (!Properties.Settings.Default.SuppressNotifications)
                    DisplayBackupResultMessage(string.Format("Successfully parsed and backed up chat log to {0}", path + fileName), "Information", MessageBoxButton.OK, MessageBoxImage.Information);
            ...
            {
                if (gameClosed)
                    DisplayBackupResultMessage("An error occured while trying to automatically save the chat log.\r\n\r\nMake sure you picked a non-system directory for your backup path.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
    }
}
